...
def generate_tsv(prototxt, weights, image_ids, outfile):
    ...
    if len(missing) == 0:
        ...
        print('Already complted {:d}'.format(len(image_ids)))
    else:
                ...
                if int(image_id) in missing:
                    ...
                    if (count % 100) == 0:
                        print('{:d}/{:d} {:.3f}s (projected finish: {:.2f} hours)'
                              .format(count + 1, len(missing), t['misc'].average_time,
                                      t['misc'].average_time * (len(missing) - count) / 3600))
...
if __name__ == '__main__':
    ...
    p = Process(target=generate_tsv,
                args=(args.prototxt, args.caffemodel, image_ids, outfile))
    p.daemon = True
    p.start()
    procs.append(p)
